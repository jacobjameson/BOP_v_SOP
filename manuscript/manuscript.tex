...
Length of Stay.---We find that batch ordering has no effect on reducing
length of stay. This estimate is robust to inclusion and exclusion of
controls for baseline characteristics. This implies that the anticipated
efficiency gains from ordering tests in a batch, as opposed to waiting
for the results of one test to come back before ordering a subsequent
test, are likely washed out by the inefficiencies associated with
ordering additional unnecessary tests.

Number of Tests.---Our 2SLS findings on the resource utilization are
presented in column 2 of Table \ref{tab:2SLS}. We find large effects.
The effect of tests batching
...
several alter- native specifications probing the key identifying
assumptions of independence, exclusion, and monotonicity. We describe
each robustness check in detail in online Appendix D and provide a brief
overview here. To probe the independence assump- tion, we perform two
checks. First, we take into account potential endogeneity in diagnosing
behavior by no longer excluding any diagnosis from our analysis sample
and by replacing the original diagnosis control with the most recent
diag- nosis code observed prior to the ED visit (results are presented
in online Appendix Table G.8, column 2). Second, we replace the
physician leniency instrument with a shift-level instrument that
captures the average leniency across all physicians on shift (column 3).
To provide robustness checks to potential exclusion restriction
violations, we adjust for chief observable margins of care that may be
correlated with leniency, namely (i) decision to admit patients to the
hospital, (ii) intensity of procedures performed, (iii) quality of care
provided, and (iv) amount of opi- oids prescribed---i.e., intensive
margin (columns 4--6). To address monotonicity, we construct
physician-diagnosis-specific leniency instruments (column 7), and we
check whether our instrument has a positive first stage across key
demographic- and health-related subsamples (online Appendix Table G.12).

Across all robustness checks and main outcomes, we find magnitudes of
our estimates broadly unchanged. Precision of estimates remains
unchanged for the outcomes of long-term use and opioid-seeking behavior,
while estimates for the outcomes of opioid use disorder and opioid
overdose mortality become noisier in some specifications.
